...
impl<'a> NetworkHeaderView<'a> {
    ...
    pub fn next_hop(&self) -> u32 {
        u32::from_be_bytes([self.buffer[12], self.buffer[13], self.buffer[14], self.buffer[15]])
    }
...
impl MeshNetwork {
    ...
    pub fn route_in_place(&mut self, local_node_id: u32, frame: &mut SpaceCANFrame) -> Result<RouteAction, RustSatError> {
        ...
        if dest == local_node_id {
            info!("Packet arrived at final destination: Node {}", local_node_id);
