...
impl SpaceCANFrame {
    pub fn new(id: u32, data: HeaplessVec<u8, 256>, priority: FramePriority, ticks: u64, seq: u16) -> Self {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, RustSatError> {
        ...
        Ok(Self {
            id, data, dlc, priority, power_mode, timestamp_ticks, sequence_number, checksum, error_correction,
        })
    }
}
...
impl SpaceCANAdapter {
    ...
    pub fn transmit(&mut self, frame: &SpaceCANFrame) -> Result<(), RustSatError> {
        ...
        info!("Transmitted frame seq={} on ch={}", frame.sequence_number, channel_id);
        Ok(())
    }
